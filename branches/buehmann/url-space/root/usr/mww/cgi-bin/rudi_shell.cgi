...
cat << EOF
	<script type=text/javascript>
		...
		function RudiEdit() {
			...
			tcmd = 'script=' +
				...
				tdata + LF +
				'echo "RUDI_EOF"';
			tmp = '/freetz/shell/exec?pid=$$&onload=parent.copyOut2Cmd()&' + tcmd;
			parent.frames["shellcmd"].location.href = tmp;
		}
	</script>
	<form action="/freetz/shell/exec?pid=$$" target="shellcmd" method=POST enctype="multipart/form-data">
	...
	<table>
		<form action="/freetz/shell/upload?pid=$$" target="shellcmd" method=POST enctype="multipart/form-data">
